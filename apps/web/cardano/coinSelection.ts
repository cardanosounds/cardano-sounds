...
const CoinSelection = {
  ...
  randomImprove: (inputs: TransactionUnspentOutput[], outputs: TransactionOutputs, limit: number, preset: TransactionUnspentOutput[] = []) => {
    ...
    console.log('randomImprove: (inputs, outputs, limit, preset = [])')
    console.log(inputs, outputs, limit, preset)

    inputs.forEach(utxo => console.log(utxoToJson(utxo)))
    preset.forEach(utxo => console.log(utxoToJson(utxo)))

    
    const _minUTxOValue = BigInt(outputs.len()) * BigInt("1000000");
    ...
    if (utxoSelection.remaining.length > 0) {
      ...
      const maxFee = Value.new(
        BigNum.from_str(maxFeeInt.toString())
      );
...
function createSubSet(utxoSelection, output) {
  if (BigInt(output.coin().to_str()) < BigInt(1)) {
    ...
    for (let i = 0; i < utxoSelection.remaining.length; i++) {
      console.log('utxoSelection.remaining[i]')
      console.log(utxoSelection.remaining[i])
      console.log('output')
      console.log(output)
      let remainingUtxo: Value = 
        typeof utxoSelection.remaining[i] === 'string' 
          ? TransactionUnspentOutput.from_bytes(Buffer.from(utxoSelection.remaining[i], 'hex')).output().amount() 
          : utxoSelection.remaining[i]
      if (
        compare(remainingUtxo, output) !==
        undefined
      ) {
...
const cloneUTxOList = (utxoList) =>{
  ...
  return utxoList.map((utxo) => {
    if(typeof utxo === 'string') return TransactionUnspentOutput.from_bytes(Buffer.from(utxo, 'hex'))
    return TransactionUnspentOutput.from_bytes(utxo.to_bytes())
  });
}
/**
